...
Location API	Fused Location Provider (com.google.android.gms:play-services-location:21.3.0)
...
Run on a physical device (recommended) or emulator with a Google Play Store–enabled system image.
Note: address lookup (Geocoder) is unreliable on some emulators — testing on a physical device with an internet connection is recommended for full functionality.

How It Works
On launch, the app requests location permission if not already granted.
Tapping Set Start Point fetches the current location via FusedLocationProviderClient.getCurrentLocation(), stores it, and displays both coordinates and a reverse-geocoded address.
Once a start point exists, Set End Point becomes enabled.
Tapping Set End Point fetches a second location and calls startLocation.distanceTo(endLocation) to compute the distance in meters, displaying the result.
Project Structure
